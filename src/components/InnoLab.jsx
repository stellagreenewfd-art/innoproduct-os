  ...
  return (
    <div className="ip-card p-5 ip-fade-in">
      ...
      {expanded && (
        <div className="border-t border-ip-border pt-3 mt-3 space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-ip-text-tertiary">机制: {concept.formulationDirection?.keyBenefit}</p>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs pt-1">
            <span className="text-ip-text-tertiary">研发周期: <strong className="text-ip-text">{concept.devCycleEstimate}</strong></span>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mt-4">
        <button onClick={() => setExpanded(!expanded)} className="ip-btn-ghost text-xs flex-1">
          {expanded ? '收起详情' : '查看详情'}
        </button>
        <button onClick={() => onSelect(concept)} className="ip-btn-ghost text-xs flex-1">
          生成规格书
        </button>
        <button onClick={() => onAddToDev(concept)} className="ip-btn-primary text-xs flex-1">
          加入开发
        </button>
      </div>
    </div>
...
export default function InnoLab() {
  const { category, concepts, loading, setSelectedConcept, setActiveModule, specData, setSpecData } = useApp()
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-xs text-ip-text-tertiary mt-1">生成5个差异化产品方案，请稍候</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 ip-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setActiveModule('trend')} className="ip-btn-ghost text-sm">
          ← 返回趋势雷达
        </button>
      </div>

      {/* Innovation level legend */}
      ...
      <div className="flex justify-center pt-2">
        <p className="text-sm text-ip-text-tertiary">
          选择一个概念 → 生成详细产品规格书，或直接加入开发追踪
        </p>
      </div>
    </div>
  )
